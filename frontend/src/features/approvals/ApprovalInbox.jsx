...
export default function ApprovalInbox({ currentUser, onNavigate, onOpenDocument }) {
  ...
  async function load() {
    ...
    try {
      const [queueRes, poRes, prRes, srRes, purRes, ccRes, amdRes, blRes] = await Promise.all([
        ...
        grab("Ringkasan Antrean", axios.get(`${API}/approvals/backlog`)),
      ]);
  ...
  /**
   * Berapa dari total antrean yang TIDAK diurus daftar layar ini (PR, spesifikasi/
   * sample, pesanan khusus, antar-PT di atas ambang, dan mesin generik). Angka ini
   * yang dulu hilang: layar ini berkata "tidak ada yang menunggu" sementara pekerjaan
   * itu duduk di tab sebelah tanpa satu pun petunjuk.
   */
