...
def g6_backlog_satu_pintu(cl):
    ...
    # Koleksi tiap baris harus ADA (kelas bug `amendments` vs `doc_amendments`).
    ...
    salah = [f"{k}→{c}" for k, _l, _v, c, _q in abl.QUEUES
             if c not in ada and c != "approval_requests"]
    ok(not salah, "setiap baris antrean menyebut koleksi yang ADA (bukan nol semu)",
       f"salah={salah}")
